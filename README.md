...
## Life Design Studio Web（阶段 1.1）

仓库正在增加一个移动端优先、可中断恢复的 Web 体验。当前版本已经覆盖四阶段、八份标准素材，并把第一阶段升级成不依赖模型的“引导式教练”：先给选择和例子，再陪用户聚焦困扰、回忆真实时刻、识别感受、区分现实边界，最后确认一份可编辑的小结。
...
打开 <http://localhost:3000>。第一阶段每次只处理一个小问题：
...
后续继续完成工作观、人生观、能量地图、三套五年方案和低成本原型。每个微步骤都会先写入本机 IndexedDB 再推进，返回不会清空答案，关闭页面后重新打开会回到准确断点。八份素材全部齐全前不会出现成果导出；完成后导出的是不含 API Key 的结构化人生设计素材，不冒充最终蓝图。
...
阶段 1.1 不调用模型、不收费、也不上传云端。当前先用确定性规则保证任何人不配置 API Key 也能获得完整引导；第一阶段通过体验验收后，再将同样的结构复制到指南针、寻路和多种可能，并在规则引导之上接入可选的 BYOK 动态追问与《个人人生设计蓝图》生成。
